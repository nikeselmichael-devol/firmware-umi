 ...
 *   UDP application payload = CBOR( [STAT, CH1..CH11] )  +  CRC8 (1 byte)
 ...
 *
 *   Two arms stream to the same PC/port; ARM_ID (CH1, set per-device below)
 *   is how the host tells the two streams apart.
 *
 * Channel mapping (CH1..CH11), all encoded as CBOR ints, fixed-point scaled:
 *   CH1  = arm_id                          1 BYTE
 *   CH2  = timestamp (ms, millis())        4 BYTE
 *   CH3  = sequence (32-bit, wraps)        4 BYTE
 *   CH4  = Encoder1 COUNT (raw, 0..16383)  4 BYTE
 *   CH5  = Encoder2 COUNT (raw, 0..16383)  4 BYTE
 *   CH6  = Accel X          (0.0001 g units,  int32)
 *   CH7  = Accel Y          (0.0001 g units,  int32)
 *   CH8  = Accel Z          (0.0001 g units,  int32)
 *   CH9  = Gyro X           (0.001 dps units, int32)
 *   CH10 = Gyro Y           (0.001 dps units, int32)
 *   CH11 = Gyro Z           (0.001 dps units, int32)
 *
...
// Unique per arm — set to 1 on arm #1's board, 2 on arm #2's board, etc.
// Sent as CH1 so the PC can tell the two arms' UDP streams apart.
static const uint8_t ARM_ID = 1;
...
static const int PIN_CLK  = 4;  
// Per-device chip selects
static const int PIN_CS_IMU  = 10;  // CS1 -> ICM42688P
static const int PIN_CS_ENC1 = 2;   // CS2 -> AS5047P #1
...
struct SensorFrame {
  uint32_t stat;
  uint8_t  armId;      // CH1
  uint32_t timestamp;  // CH2
  uint32_t sequence;   // CH3
  int32_t  enc1Count;  // CH4
  int32_t  enc2Count;  // CH5
  int32_t  accelX, accelY, accelZ; // CH6-8
  int32_t  gyroX, gyroY, gyroZ;    // CH9-11
};

size_t buildPacket(const SensorFrame& f, uint8_t* out, size_t outCap) {
  CborWriter w(out, outCap);
  w.writeArrayHeader(12);      // [stat, ch1..ch11]
  w.writeUInt(f.stat);
  w.writeUInt(f.armId);
  w.writeUInt(f.timestamp);
  w.writeUInt(f.sequence);
  w.writeInt(f.enc1Count);
  w.writeInt(f.enc2Count);
  w.writeInt(f.accelX);
  w.writeInt(f.accelY);
  w.writeInt(f.accelZ);
  w.writeInt(f.gyroX);
...
uint8_t seq = 0;          // rolling 8-bit sequence packed into STAT bits8-15
uint32_t seqCounter = 0;  // full 32-bit sequence sent as CH3
...
void publishFrame() {
  ...
  f.armId = ARM_ID;
  f.timestamp = millis();
  f.sequence = seqCounter++;

  // raw encoder counts, 0..16383 over 360deg
  f.enc1Count = lastEnc1Raw;
  f.enc2Count = lastEnc2Raw;

  // accel raw -> g *10000 fixed point. +-16g => 2048 LSB/g.
  const float accelLsbPerG = 2048.0f;
  f.accelX = (int32_t)((lastImu.ax / accelLsbPerG) * 10000.0f);
  f.accelY = (int32_t)((lastImu.ay / accelLsbPerG) * 10000.0f);
  f.accelZ = (int32_t)((lastImu.az / accelLsbPerG) * 10000.0f);
  ...
  f.gyroX = (int32_t)((lastImu.gx / gyroLsbPerDps) * 1000.0f);
